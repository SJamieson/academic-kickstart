...
<PipelineReliability caption="Drag the difficulty slider. The uncertainty-aware system (teal) holds its end-to-end reliability by escalating the uncertain minority; the confident system (grey) degrades as inputs get harder — roughly neutral on easy inputs, a wide margin on hard ones. (Synthetic 3-stage pipeline with correlated errors.)" />
